...
<body>
	...
	<div class="col-lg-6">
	  <div class="panel panel-primary">
            <div class="panel-heading">
			<center>GRADE 7 REPRESENTATIVE</center>
            </div>
            <div class="panel-body" style = "background-color:;">
				<?php
					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'Grade 7 Representative'") or die(mysqli_errno());
					while($fetch = $query->fetch_array())
					{
				?>
						<div id = "position">
						<div class="flx-cntner" >
								...
								<div>
									<center><input type = "checkbox" style = "height: 30px; width: 30px;" value = "<?php echo $fetch['candidate_id'] ?>" name = "g7rep_id" class = "g7rep"></center>
								</div>
							</div>
						</div>
...
<?php include ('script.php')?>
  ...
  <script type = "text/javascript">
		$(document).ready(function(){
			...
			$(".g7rep").on("change", function(){
				if($(".g7rep:checked").length == 1)
				{
					$(".g7rep").attr("disabled", "disabled");
					$(".g7rep:checked").removeAttr("disabled");
				}
			else
				{
					$(".g7rep").removeAttr("disabled");
				}
			});
		});	
	</script>
