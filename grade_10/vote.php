...
<body>
	...
	<div class="col-lg-6">
	  <div class="panel panel-primary">
            <div class="panel-heading">
			<center>GRADE 10 REPRESENTATIVE</center>
            </div>
            <div class="panel-body" style = "background-color:;">
				<?php
					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'Grade 10 Representative'") or die(mysqli_errno());
					while($fetch = $query->fetch_array())
					{
				?>
						<div id = "position">
						<div class="flx-cntner" >
								...
								<div>
									<center><input type = "checkbox" style = "height: 30px; width: 30px;" value = "<?php echo $fetch['candidate_id'] ?>" name = "g10rep_id" class = "g10rep"></center>
								</div>
							</div>
						</div>
...
<?php include ('script.php')?>
  ...
  <script type = "text/javascript">
		$(document).ready(function(){
			...
			$(".g10rep").on("change", function(){
				if($(".g10rep:checked").length == 1)
				{
					$(".g10rep").attr("disabled", "disabled");
					$(".g10rep:checked").removeAttr("disabled");
				}
			else
				{
					$(".g10rep").removeAttr("disabled");
				}
			});
		});	
	</script>
